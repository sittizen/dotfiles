...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-tools query`. Use `node /home/simone.cittadini@gruppomol.lcl/.config/opencode/get-shit-done/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-tools query` where a handler exists.
...
```bash
node "/home/simone.cittadini@gruppomol.lcl/.config/opencode/get-shit-done/bin/gsd-tools.cjs" graphify build
```
